...
from main import (
    ...
    save_NanoClaw_config,
    ensure_config_structure,
    format_model_for_NanoClaw,
    NanoClaw_CONFIG_PATH
)


# Constants
STATE_FILE = Path.home() / ".NanoClaw" / ".freeride-watcher-state.json"
...
def rotate_to_next_model(api_key: str, state: dict, reason: str = "manual"):
    ...
    # Extract base model ID from NanoClaw format
    current_base = None
    if current:
    ...
    formatted_primary = format_model_for_NanoClaw(next_model, with_provider_prefix=True)
    config["agents"]["defaults"]["model"]["primary"] = formatted_primary

    # Add to models allowlist
    formatted_for_list = format_model_for_NanoClaw(next_model, with_provider_prefix=False)
    ...
    for m in models:
        ...
        fb_formatted = format_model_for_NanoClaw(m["id"], with_provider_prefix=False)
