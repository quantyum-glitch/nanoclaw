...
NanoClaw_CONFIG_PATH = Path.home() / ".NanoClaw" / "CLAUDE.md"
CACHE_FILE = Path.home() / ".NanoClaw" / ".freeride-cache.json"
...
def get_api_key() -> Optional[str]:
    """Get OpenRouter API key from environment or NanoClaw config."""
    ...
    if NanoClaw_CONFIG_PATH.exists():
        try:
            config = json.loads(NanoClaw_CONFIG_PATH.read_text())
            # Check env section
...
def load_NanoClaw_config() -> dict:
    """Load NanoClaw configuration."""
    if not NanoClaw_CONFIG_PATH.exists():
        return {}

    try:
        return json.loads(NanoClaw_CONFIG_PATH.read_text())
    except json.JSONDecodeError:
        return {}


def save_NanoClaw_config(config: dict):
    """Save NanoClaw configuration."""
    NanoClaw_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
    NanoClaw_CONFIG_PATH.write_text(json.dumps(config, indent=2))


def format_model_for_NanoClaw(model_id: str, with_provider_prefix: bool = True) -> str:
    """Format model ID for NanoClaw config.

    NanoClaw uses two formats:
    ...
    # prefix NanoClaw adds AND the actual provider name in the API model ID.
    # The API model ID is "openrouter/free" (no :free suffix â€” it's a router, not a free-tier model).
...
def get_current_model(config: dict = None) -> Optional[str]:
    """Get currently configured model in NanoClaw."""
    if config is None:
        config = load_NanoClaw_config()
...
def update_model_config(
    ...
    """Update NanoClaw config with the specified model.
    ...
    formatted_primary = format_model_for_NanoClaw(model_id, with_provider_prefix=True)
    formatted_for_list = format_model_for_NanoClaw(model_id, with_provider_prefix=False)
    ...
    if add_fallbacks:
        ...
        if api_key:
            ...
            free_router_primary = format_model_for_NanoClaw("openrouter/free", with_provider_prefix=True)
            ...
            for m in free_models:
                ...
                m_formatted = format_model_for_NanoClaw(m["id"], with_provider_prefix=False)
                m_formatted_primary = format_model_for_NanoClaw(m["id"], with_provider_prefix=True)
...
def cmd_list(args):
    ...
    for i, model in enumerate(models[:limit], 1):
        ...
        # Check status
        formatted = format_model_for_NanoClaw(model_id, with_provider_prefix=True)
        formatted_fallback = format_model_for_NanoClaw(model_id, with_provider_prefix=False)
...
def cmd_switch(args):
    ...
    if update_model_config(
        ...
        if as_fallback:
            ...
            print("Success! NanoClaw config updated.")
        ...
        print("Error: Failed to update NanoClaw config.")
        sys.exit(1)
...
def cmd_status(args):
    ...
    # NanoClaw config path
    print(f"\nNanoClaw Config: {NanoClaw_CONFIG_PATH}")
    print(f"  Exists: {'Yes' if NanoClaw_CONFIG_PATH.exists() else 'No'}")
...
def cmd_fallbacks(args):
    ...
    for m in models:
        formatted = format_model_for_NanoClaw(m["id"], with_provider_prefix=False)
        formatted_primary = format_model_for_NanoClaw(m["id"], with_provider_prefix=True)
    ...
    print("\nWhen rate limited, NanoClaw will automatically try these models.")
    print("Restart NanoClaw for changes to take effect.")


def main():
    parser = argparse.ArgumentParser(
        prog="freeride",
        description="FreeRide - Free AI for NanoClaw. Manage free models from OpenRouter."
    )
